...
from uix.labels import Label, ChemistryLabel
...
class Chemistry(BoxLayout):
    ...
    def reading_elements_info(self, *args):
        chemistry_grand_title = Label('Chemistry', 'Chimie', 'Химия', 'Քիմիա')
        chemistry_grand_title.font_size += 15

        self.element_enter = TextInput(hint_text='Enter element name', size_hint_y=.5, pos_hint={'top': 1})
        self.read_el_info = Button(text='Search', size_hint=(.2, .5), pos_hint={'top': 1}, on_press=self.get_element_info)
        ...
        self.add_widget(chemistry_grand_title)
    ...
    def get_element_info(self, *arg):
        ...
        self.reply_layout.add_widget(ChemistryLabel(self.get_element, element_charge, element_electronegativity, element_Ar, element_type), index=self.index)
        self.index += 1